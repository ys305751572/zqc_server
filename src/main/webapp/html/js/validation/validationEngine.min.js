 ...
 */
 (function($) {
	 ...
	 var methods = {
		...
		* During this process all the (field, value) pairs are sent to the service which returns a list of invalid fields or true
		*
		* @param {jqObject} form
		* @param {Map} options
		*/
			...
			$.ajax({
				...
				success: function(json) {
					if (json !== true) {
						...
						// the service may return green or closing prompt actions
			...
			for (var i = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "groupRequired":
						...
						break;
					case "number" :
						errorMsg = methods._getErrorMessage(form, field, rules[i], rules, i, options, methods._number);
						break;
					case "ajax":
		...
		*/
		_minSize: function(field, rules, i, options) {
		  ...
		  * @return an error string if validation failed
		  */
		 _number: function(field, rules, i, options) {
			 var rule = options.allrules.number;
			 var alertText= options.allrules.number.alertText;
			 var regex = rule.regex;
			 var fieldVar = field.val();
			 if(!regex.test(fieldVar)) {
				return alertText;
			 }
		 },
			 ...
			 if (!options.isError && !methods._checkAjaxFieldStatus(field.attr("id"), options)) {
				 $.ajax({
					 ...
					 success: function(json) {
						 ...
						 // asynchronously called on success, data is the json answer from the service
						 ...
						 if (errorField.length == 1) {
							 var status = json[1];
							 // read the optional msg from the service
							 var msg = json[2];
							 if (!status) {
		...
		*/
		_ajaxError: function(data, transport) {
			if(data.status == 0 && transport == null)
				alert("The page is not served from a service! ajax call failed");
